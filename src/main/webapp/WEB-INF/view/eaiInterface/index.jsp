...
<script type="text/javascript">
    ...
    var searchBtn = function(){
        ...
        location.href = "/eaiInterface/index?searchVal=" + encodeURIComponent(searchVal);
    }
	...
	<div class="container">
            ...
            <div class="page__data-wrap">
                <div class="page__data-util--box">
                    <p class="page__data-cnt">Total : ${eaiOverView.totalElements} items</p>
					<div class="btn_group">
						<a href="/eaiInterface/create" class="btn__white">NEW</a>
                ...
                <div class="page__data-list">
                    <table>
                        ...
                        <c:forEach items="${eaiOverView.content}" var="overView" varStatus="status">
                            <tr onclick="goToDetailPage(${overView.eaiSeq})" style="cursor: pointer;" >
                                <td>${eaiOverView.number * eaiOverView.size + status.index + 1}</td>
                                <td>${overView.eaiId}</td>
                                <td>${overView.eaiNm}</td>
                                <td>${overView.eaiDivCd}</td>
                                <td>${overView.eaiDesc}</td>
                            </tr>
                        </c:forEach>
                    </table>
                    <ul class="pagination">
                            ...
                            <c:forEach var="i" begin="${startPageNo}" end="${endPageNo}">
                                <c:choose>
                                    <c:when test="${i-1 eq eaiOverView.number}">
                            ...
                            <!-- 끝 페이지 링크 -->
                            <c:choose>
                                <c:when test="${eaiOverView.last}">
                                    <li class="disabled"><a href="#">끝 &raquo; </a></li> <!-- 마지막 페이지일 때 비활성화 -->
                                </c:when>
                                <c:otherwise>
                                    <li><a href="?page=${eaiOverView.totalPages - 1}">끝 &raquo; </a></li> <!-- 마지막 페이지로 이동 -->
                                </c:otherwise>
                            </c:choose>
                    </ul>
                </div>
